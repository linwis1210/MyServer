...
void HttpRequest::parsePath()
{
    ...
    }else if(path.substr(0, 11) == "/inference."){
        //下载文件
        int index = stoi(path.substr(14,15));
        vector<string> files = getFiles("./files");
        inference(files[index]);
    }  
}
...
void HttpRequest::inference(string files_name){
    string root_name = "./files/";
    files_name = root_name + files_name;
    int size = files_name.size();
    if (files_name.substr(size-4, size-1) == ".jpg" || files_name.substr(size-4, size-1) == ".png")
    {
        infer inf;
        inf.getResult(files_name);
        path = "/result.html";
    }else
    {
        path = "/error.html";
